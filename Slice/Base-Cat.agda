...
SL→∘ : (X : Set) → {IA JB KC : SL X} → SL→ X JB KC →  SL→ X IA JB → SL→ X IA KC
SL→∘ X g f i =  proj₁ (g (proj₁ (f i))) ,
  trans (proj₂ (f i)) (proj₂ (g (proj₁ (f i))))


SL-∘≡ : (X : Set) → {IA JB KC : SL X} → {g g' : SL→ X JB KC} →  {f f' : SL→ X IA JB}
  → SL→≡ X _ _ g g' → SL→≡ X _ _ f f' → SL→≡ X _ _ (SL→∘ X g f) (SL→∘ X g' f')
SL-∘≡ X {_} {_} {_} {g} {g'} {f} {f'} g≡g' f≡f' i =
  trans (cong (λ z → proj₁ (g z)) (f≡f' i)) (g≡g' (proj₁ (f' i)))

-- Category properties
SL-asso : (X : Set) → {IA JB KC LD : SL X}
  → (f : SL→ X IA JB) → (g : SL→ X JB KC) → (h : SL→ X KC LD)
  → SL→≡ X IA LD (SL→∘ X {IA} {KC} {LD} h (SL→∘ X {IA} {JB} {KC} g f))
                 (SL→∘ X {IA} {JB} {LD} (SL→∘ X {JB} {KC} {LD} h g) f)
SL-asso X f g h i = refl

SL-luni : (X : Set) → {IA JB : SL X} → (f : SL→ X IA JB)
  → SL→≡ X IA JB (SL→∘ X {IA} {IA} {JB} f (SL→id X IA)) f
SL-luni X f i = refl

SL-runi : (X : Set) → {IA JB : SL X} → (f : SL→ X IA JB)
  → SL→≡ X IA JB (SL→∘ X {IA} {JB} {JB} (SL→id X JB) f) f
SL-runi X f i = refl
...
SL-iso X IA JB = Σ (SL→ X IA JB) λ f → Σ (SL→ X JB IA) λ g →
    SL→≡ X IA IA (SL→∘ X {IA} {JB} {IA} g f) (SL→id X IA)
  × SL→≡ X JB JB (SL→∘ X {JB} {IA} {JB} f g) (SL→id X JB)
...
SL-Cat X = record
   ...
   ; id = SL→id X _
   ; _∘_ = SL→∘ X
   ; assoc = λ {A B C D f g h} → (SL-asso X f g h)
   ; sym-assoc = λ {A B C D f g h} → (SL-asso X f g h)
   ; identityˡ = λ {A B f} → SL-luni X f
   ; identityʳ = λ {A B f} → SL-runi X f
   ; identity² = λ i → refl
   ; equiv = record
     { refl = λ i → refl
     ; sym = λ x i → sym (x i)
     ; trans = λ x y i → trans (x i) (y i)
     }
   ; ∘-resp-≈ = λ {A B C g g' f f'} → SL-∘≡ X {A} {B} {C} {g} {g'} {f} {f'}
   }
